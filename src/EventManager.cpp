...
void EventManager::initRotatingLights() {
    ...
    moons.push_back(Model("fileRecources/Models/moon/moon.obj", renderer->getDefaultShader()));
...
void EventManager::initSolarSystem() {
    earth = new Model("fileRecources/Models/earth/earth.obj", renderer->getDefaultShader());
    earth->setShininess(6.0f);
    earth->setLightingSensitivity(1.0f, 0.5f);
    sun = new Model("fileRecources/Models/sun/sun.obj", renderer->getDefaultShader());
    ...
    moon = new Model("fileRecources/Models/moon/moon.obj", renderer->getDefaultShader());
    moon->setShininess(4.0f);
    moon->setLightingSensitivity(1.0f, 0.5f);

    containerTex = Renderer::loadTexture("fileRecources/Textures/container2.png", false, true);
    specularMap = Renderer::loadTexture("fileRecources/Textures/container2_specular.png", false, true);
}
...
void EventManager::initEvents() {
      initLights();
      singleColorShader = renderer->createShader("fileRecources/Shaders/VertexShader.vert", "fileRecources/Shaders/SingleColorFragShader.frag", true, true);//shader for outlines TODO move to drawable

      if (renderCubeOfCubes == true || drawTestCube == true) {
          containerTex = Renderer::loadTexture("fileRecources/Textures/container2.png", false, true);
          specularMap = Renderer::loadTexture("fileRecources/Textures/container2_specular.png", false, true);
          smileyTex = Renderer::loadTexture("fileRecources/Textures/awesomeface.png", true, true);
      }
      ...
      if (renderBackPack == true) {
          backpack = new Model("fileRecources/Models/backpack/backpack.obj", renderer->getDefaultShader());
      }
      ...
      skyboxShader = renderer->createShader("fileRecources/Shaders/SkyboxVertShader.vert", "fileRecources/Shaders/SkyboxFragShader.frag", true, false);
      string directory("fileRecources/Textures/skyboxes_space/blue/");
